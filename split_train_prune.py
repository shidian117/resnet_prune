...
add_arg('lr',                       float,  0.001,                                      "The learning rate used to fine-tune pruned model.")
...
add_arg('model_save_dir',           str,    "./split_model_prune_het_quant",                           "model save directory.")
add_arg('pruned_ratio1',             float,    0.2,                                           "The ratios to be pruned.")
add_arg('pruned_ratio2',             float,    0.1,                                           "The ratios to be pruned.")
...
def compress(args):
   ...
   # _logger.info("Origin model summary:")
    ...
    if args.criterion == 'fpgm':
        pruner1 = paddleslim.dygraph.FPGMFilterPruner(net1, [1] + image_shape)
        pruner2 = paddleslim.dygraph.FPGMFilterPruner(net2, [1] + temp_shape)
    elif args.criterion == 'l1_norm':
        pruner1 = paddleslim.dygraph.L1NormFilterPruner(net1, [1] + image_shape)
        pruner2 = paddleslim.dygraph.L1NormFilterPruner(net2, [1] + temp_shape)


    params1 = get_pruned_params(args, net1)
    params2 = get_pruned_params(args, net2)
    ratios1 = {}
    ratios2 = {}
    for param in params1:
        ratios1[param] = args.pruned_ratio1
    for param in params2:
        ratios2[param] = args.pruned_ratio2
    plan1 = pruner1.prune_vars(ratios1, [0])
    plan2 = pruner2.prune_vars(ratios2, [0])
    ...
    quanter1 = QAT(config=quant_config)
    quanter2 = QAT(config=quant_config)
    quanter1.quantize(net1)
    quanter2.quantize(net2)
    ...
    if paddle.distributed.get_rank() == 0:
        ...
        #paddle.jit.save(net2, model_prefix4, input_spec=[paddle.static.InputSpec(shape=[None, 128, 3, 3], dtype='float32')])
        

        model_prefix5 = os.path.join(args.model_save_dir, "client_model_infer_qat")
        model_prefix6 = os.path.join(args.model_save_dir, "server_model_infer_qat")
        quanter1.save_quantized_model(net1,model_prefix5,input_spec=[paddle.static.InputSpec(shape=[None, 3, 32, 32], dtype='float32')])
        quanter2.save_quantized_model(net2,model_prefix6,input_spec=[paddle.static.InputSpec(shape=[None, 128, 3, 3], dtype='float32')])
